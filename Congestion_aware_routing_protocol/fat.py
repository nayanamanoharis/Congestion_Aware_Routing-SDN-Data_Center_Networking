...
class FatTopo(Topo):
    def __init__(self):
        ...
        self.hosts_ = [
                self.addHost('h%d' % (hostId+1), ip=('10.0.0.%d/27' % (hostId+1)))
                for hostId in range(16)]

        self.edge_switches_ = [
                self.addSwitch('es%d' % switchId, dpid=("%0.2X" % (switchId+1)), protocols='OpenFlow13')
                for switchId in range(8)]
        
        self.aggregate_switches_ = [
                self.addSwitch('as%d' % switchId, dpid=("%0.2X" % (switchId+9)), protocols='OpenFlow13')
                for switchId in range(8)]

        self.core_switches_ = [
                self.addSwitch('cs%d' % switchId, dpid=("%0.2X" % (switchId+17)), protocols='OpenFlow13')
                for switchId in range(4)]

        self.hostLinks_odd_ = [
                self.addLink('h%d' % ((2*eId)+2), 'es%d' % (eId),0,2)
                for eId in range(8)]
 
        self.hostLinks_even_ = [
                self.addLink('h%d' % ((2*eId)+1), 'es%d' % (eId),0,1)
                for eId in range(8)]
